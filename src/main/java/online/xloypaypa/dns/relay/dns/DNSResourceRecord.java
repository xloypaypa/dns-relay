 ...
 * 
 */
public final class DNSResourceRecord implements ByteAble {
	...
	public byte[] toBytes() {
		...
		ByteBuffer byteBuffer = ByteBuffer.allocate(name.length + 10 + this.rdata.length);
		byteBuffer.order(ByteOrder.BIG_ENDIAN);
		byteBuffer.put(name);
		byteBuffer.putShort(this.rtype);
		byteBuffer.putShort(this.rclass);
		byteBuffer.putInt(this.ttl);
		byteBuffer.putShort((short) this.rdata.length);
		byteBuffer.put(this.rdata);
